...
export class KanbanMetricsCalculationService {
  ...
  public static async calculateMetricsForBoard(kanbanBoardId: number): Promise<KanbanMetrics | null> {
    try {
      ...
      const [kanbanMetrics, created] = await KanbanMetrics.upsert({
        ...
        // Cycle Time Metrics
        averageCycleTime: timeMetrics.averageCycleTime,
        medianCycleTime: timeMetrics.medianCycleTime,
        cycleTimes: timeMetrics.cycleTimes,
        
        // Lead Time Metrics
        averageLeadTime: timeMetrics.averageLeadTime,
        medianLeadTime: timeMetrics.medianLeadTime,
        ...
        // Age Metrics
        averageAgeInProgress: ageMetrics.averageAgeInProgress,
        oldestIssueAge: ageMetrics.oldestIssueAge,
  ...
  private static calculateTimeMetrics(issues: KanbanIssue[]) {
    ...
    logger.info(`⏱️ Time metrics - Avg Cycle: ${avgCycleTime?.toFixed(1)}d, Avg Lead: ${avgLeadTime?.toFixed(1)}d (${cycleTimes.length} completed issues)`);
  ...
  private static calculateAgeMetrics(issues: KanbanIssue[]) {
    ...
    logger.info(`📅 Age metrics - Average: ${averageAge.toFixed(1)}d, Oldest: ${oldestAge}d (${inProgressIssues.length} in-progress issues)`);
